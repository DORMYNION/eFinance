    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('loan_amount') ? 'is-invalid' : '' }}" name="loan_amount_id" id="loan_amount_id" required>
                    @foreach($loan_amounts as $id => $loan_amount)
                        <option value="{{ $id }}" {{ old('loan_amount_id') == $id ? 'selected' : '' }}>{{ $loan_amount }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('paid_at') ? 'is-invalid' : '' }}" type="text" name="paid_at" id="paid_at" value="{{ old('paid_at') }}" required>
